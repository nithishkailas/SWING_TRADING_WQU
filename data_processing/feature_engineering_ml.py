...
#labeled_data = pd.read_csv('SBIN_labeled.csv', index_col='Date',parse_dates=True,infer_datetime_format=True)
...
for i in range(0,len(raw_data.index)):
    ...
    features.at[features.index[i], '60min_above_BBU_14_2'] = 1 if raw_data.loc[raw_data.index[i],'Close']  > raw_data.loc[raw_data.index[i],'60min_BBU_14_2'] else 0
    features.at[features.index[i], '60min_below_BBL_14_2'] = 1 if raw_data.loc[raw_data.index[i],'Close']  < raw_data.loc[raw_data.index[i],'60min_BBL_14_2'] else 0
    features.at[features.index[i], '60min_above_BBU_14_3'] = 1 if raw_data.loc[raw_data.index[i],'Close']  > raw_data.loc[raw_data.index[i],'60min_BBU_14_3'] else 0
    features.at[features.index[i], '60min_below_BBL_14_3'] = 1 if raw_data.loc[raw_data.index[i],'Close']  < raw_data.loc[raw_data.index[i],'60min_BBL_14_3'] else 0
    features.at[features.index[i], '60min_above_BBU_20_2'] = 1 if raw_data.loc[raw_data.index[i],'Close']  > raw_data.loc[raw_data.index[i],'60min_BBU_20_2'] else 0
    features.at[features.index[i], '60min_below_BBL_20_2'] = 1 if raw_data.loc[raw_data.index[i],'Close']  < raw_data.loc[raw_data.index[i],'60min_BBU_20_2'] else 0
    ...
    if raw_data.loc[raw_data.index[i],'60min_MA_5'] > raw_data.loc[raw_data.index[i],'60min_MA_8'] > raw_data.loc[raw_data.index[i],'60min_MA_14'] > raw_data.loc[raw_data.index[i],'60min_MA_20']:
        features.at[features.index[i], '60min_sma_align'] = 1
...
'60min_ADX_14','60min_AROONOSC_14','60min_BOP','60min_CCI_14','60min_DMI_14','60min_RSI_14','60min_slowk','60min_slowd','60min_fastk','60min_fastd','60min_WILLR_14','60min_HT_TRENDMODE']

indicators = raw_data[indicator_col_list]

features_full = pd.concat([features,indicators],axis=1)

features_full.to_csv('SBIN_features.csv')
